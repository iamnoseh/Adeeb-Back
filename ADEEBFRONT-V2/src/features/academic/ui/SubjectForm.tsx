...
export function SubjectForm({ subjectId }: SubjectFormProps) {
  ...
  const form = useForm<SubjectFormValues, unknown, SubjectFormValues>({
    resolver: zodResolver(
      createSubjectFormSchema(t),
    ) as unknown as Resolver<SubjectFormValues>,
    defaultValues: {
      nameTg: "",
      nameRu: "",
      nameEn: "",
  ...
  useEffect(() => {
    if (subjectQuery.data) {
      const byLanguage = new Map(
        subjectQuery.data.translations.map((item) => [item.language, item]),
      );
      form.reset({
  ...
  return (
    <form
      ...
      onSubmit={(event) =>
        void form.handleSubmit((values: SubjectFormValues) =>
    ...
    >
      ...
      <FormField
        label={isEdit ? t("subjectIconNew") : t("subjectIcon")}
