...
export function TopicsPage() {
  ...
  const topicQueryParams = { page, pageSize: 10, search: search || undefined, subjectId: subjectId || undefined }
          ...
          <tbody>{topics.map((topic) => (
            <tr key={topic.id} className="border-t border-[var(--border)]">
              ...
              <td className="px-4 py-3"><div className="flex justify-end gap-2">
                <Link className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--text)] no-underline hover:bg-[var(--surface-muted)]" to={`/admin/topics/${topic.id}/edit`}><Edit className="h-4 w-4" /> {t('edit')}</Link>
                <Button variant="danger" className="px-3" disabled={topic.status === 2} onClick={() => { if (window.confirm(t('confirmDelete'))) removeMutation.mutate(topic.id) }}><Trash2 className="h-4 w-4" /> {t('delete')}</Button>
              </div></td>
            </tr>
          ))}</tbody>
        </Table></TableShell>
