...
export function TopicForm({ topicId }: TopicFormProps) {
  ...
  const form = useForm<TopicFormValues, unknown, TopicFormValues>({
    resolver: zodResolver(
      createTopicFormSchema(t),
    ) as unknown as Resolver<TopicFormValues>,
    defaultValues: {
      subjectId: "",
      displayOrder: 0,
      status: 1,
  ...
  useEffect(() => {
    ...
    const byLanguage = new Map(
      existingTopic.translations.map((item) => [item.language, item]),
    );
    form.reset({
  ...
  const mutation = useMutation({
    mutationFn: (values: TopicFormValues) =>
      topicId ? topicsApi.update(topicId, values) : topicsApi.create(values),
    onSuccess: async () => {
  ...
  return (
    <form
      className="app-surface mx-auto grid w-full max-w-5xl gap-6 rounded-[2rem] p-5 md:p-7"
      onSubmit={(event) =>
        void form.handleSubmit((values: TopicFormValues) =>
    ...
    >
      ...
      <section className="rounded-[1.5rem] bg-[var(--surface-soft)] p-5 ring-1 ring-[var(--border)]">
        <h2 className="text-base font-bold">
          {topicId ? t("editTopic") : t("newTopic")}
        </h2>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <FormField
          label={t("topicNameTg")}
          error={form.formState.errors.nameTg?.message}
        >
