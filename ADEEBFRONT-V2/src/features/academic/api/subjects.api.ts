...
import type {
  ...
  SubjectFormValues,
  SubjectResponse,
} from "@/features/academic/model/academic.types";
import { getStoredUiLanguage } from "@/shared/i18n/language";

export const subjectKeys = {
  list: (query: AcademicListQuery = {}) =>
    ["subjects", "list", query, getStoredUiLanguage()] as const,
  publicList: (query: AcademicListQuery = {}) =>
    ["subjects", "public-list", query, getStoredUiLanguage()] as const,
  detail: (id: string) =>
    ["subjects", "detail", id, getStoredUiLanguage()] as const,
};

export const subjectsApi = {
  async list(query: AcademicListQuery = {}) {
    const response = await httpClient.get<PagedResponse<SubjectResponse>>(
      `/api/v2/admin/subjects${toQueryString(query)}`,
    );
    return response.data;
  },
  async publicList(query: AcademicListQuery = {}) {
    const response = await httpClient.get<PagedResponse<SubjectResponse>>(
      `/api/v2/subjects${toQueryString(query)}`,
    );
    return response.data;
  },
  async detail(id: string) {
    const response = await httpClient.get<SubjectResponse>(
      `/api/v2/admin/subjects/${id}`,
    );
    return response.data;
  },
  async create(values: SubjectFormValues) {
    const body = toSubjectFormData(values);
    const response = await httpClient.post<SubjectResponse>(
      "/api/v2/admin/subjects",
      body,
    );
    return response.data;
  },
  async update(id: string, values: SubjectFormValues) {
    const body = toSubjectFormData(values);
    const response = await httpClient.put<SubjectResponse>(
      `/api/v2/admin/subjects/${id}`,
      body,
    );
    return response.data;
