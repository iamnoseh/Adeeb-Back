    ...
    : IClassFixture<AdeebApiFactory>, IAsyncLifetime
{
    ...
    public async Task Parallel_submit_awards_exactly_one_reward_and_one_balance_credit()
    {
        ...
        var reward = await verify.TestXpRewards.AsNoTracking().SingleAsync();
        var balance = await verify.StudentTestXpBalances.AsNoTracking().SingleAsync();
        Assert.Equal(13, reward.TotalXpUnits);
